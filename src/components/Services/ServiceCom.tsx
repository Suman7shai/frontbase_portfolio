...
const services = [
    {
        ...
        description: "Accelerate growth with strategic campaigns designed to attract and convert."
    },
    {
        icon: <IoStatsChart />,
                ...
                <div className="max-w-6xl w-full mt-8 px-4 sm:px-6 lg:px-8 animate" ref={containerRef}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {services.map((elem, index) => (
                            <div key={index} className="service-card hero w-full">
                                <ServiceCart icon={elem.icon} title={elem.title} description={elem.description} />
                            </div>
                        ))}
                    </div>
                </div>
